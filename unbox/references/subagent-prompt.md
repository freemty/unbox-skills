...
**Search queries in this prompt reference `references/search-playbook.md` as the canonical source.**
**For backfill mode, see the backfill-specific prompt at `references/backfill-prompt.md`.**
...
**Full tool reference: `references/search-playbook.md`**

### General Search (with fallback chain)

Google search is unreliable under parallel load. **Follow this fallback chain for every search:**

```
Plan A: opencli google search "query" --limit 10 -f md
  ↓ if fails (stale page, CAPTCHA, timeout)
Plan B: python3 ~/.claude/skills/web-fetcher/scripts/fetch.py "https://www.bing.com/search?q={URL_ENCODED}"
  ↓ if fails
Plan C: python3 ~/.claude/skills/web-fetcher/scripts/fetch.py "https://duckduckgo.com/?q={URL_ENCODED}"
```

**URL encoding:** `python3 -c "import urllib.parse; print(urllib.parse.quote('中文查询'))"`

**If Plan A fails on your first query, skip it entirely — use Plan B/C for all remaining searches.**

### Platform-Specific Tools (more reliable than Google)

| Task | Tool | Command |
|------|------|---------|
| 知乎搜索 | opencli | `opencli zhihu search "query" --limit 10 -f md` |
| 知乎文章下载 | opencli | `opencli zhihu download --url "URL" -f md` |
...
| YouTube搜索 | fetch.py | `python3 fetch.py "https://www.youtube.com/results?search_query={URL_ENCODED}"` |
| 搜狗微信 | fetch.py | `python3 fetch.py "https://weixin.sogou.com/weixin?type=2&query={URL_ENCODED}"` |
| 机器之心搜索 | fetch.py | `python3 fetch.py "https://www.jiqizhixin.com/search?keywords={URL_ENCODED}"` |
| Semantic Scholar | curl | `curl -s "https://api.semanticscholar.org/graph/v1/author/search?query=NAME&fields=..."` |
| GitHub | gh | `gh api users/USERNAME` |
| Fetch any URL | fetch.py | `python3 ~/.claude/skills/web-fetcher/scripts/fetch.py <url>` |
| Wayback Machine | curl+fetch | `curl -s "https://web.archive.org/cdx/search/cdx?url=URL&output=json&collapse=timestamp:6"` |

**知乎注意：** `fetch.py` 抓知乎会 403，必须用 `opencli zhihu` 系列。

**关键原则：** 对于中文内容，平台直搜（知乎 + B站 + 搜狗微信 + 机器之心）比 Google 更可靠。优先用平台工具，Google 作为补充。
