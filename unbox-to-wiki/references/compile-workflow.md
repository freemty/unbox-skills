...
**Note:** The name-matching and dedup logic follows `~/.claude/skills/unbox/references/merge-utils.md`. Wiki cross-refs use `[[entities/slug]]` links rather than `📎` markers, but the same idempotency principles apply.
...
Read the profile's mentor/collaborator names. For each name mentioned:
1. Try exact filename match: `ls ~/selfos/wiki/entities/ | grep -i "<name-fragment>"`
2. **If no match: scan aliases.** Extract all `aliases:` fields from entity frontmatter
   and check if any alias matches the name (case-insensitive, substring OK for CJK).
   ```bash
   grep -rl "aliases:.*{name}" ~/selfos/wiki/entities/ --include="*.md"
   ```
   This catches cases like "韩松" matching `song-han.md` (alias: "韩松"),
   or "Jun Gao" matching `高俊.md` (alias: "Jun Gao").
3. If found: add cross-ref link in the new entity page AND in the existing entity page.
