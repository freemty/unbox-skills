...
**Suggest mode has moved to `/unbox-next --suggest`.**
...
aliases: ["{Chinese Name}", "{English Name}", "{pinyin}", "{nickname}", "{github-username}"]
...
**This step uses the same merge protocol as `/unbox-cross-ref`. See `~/.claude/skills/unbox/references/merge-utils.md` for the canonical append/dedup rules.**

The cross-reference scan here is wiki-specific: it creates `[[entities/slug]]` links in wiki pages, not `📎` markers in profiles. The underlying name-matching logic is shared.
...
- Check if `~/selfos/wiki/entities/{name-slug}.md` exists (exact filename match)
- **If no exact match: check aliases.** Entity pages have `aliases:` in frontmatter
  listing all name variants (Chinese, English, pinyin, nicknames, GitHub usernames).
  Search: `grep -rl "aliases:.*{name}" ~/selfos/wiki/entities/`
- If found by either method: add `[[entities/{name-slug}]]` cross-ref in both directions
...
When enriching existing data, follow this order:
1. **First**: `/unbox --mode=backfill` — fill gaps in raw profiles from web searches
2. **Then**: `/unbox-cross-ref` — propagate internal cross-references
3. **Then**: `/unbox-to-wiki --inject` — sync enriched profile data into wiki

Running inject before backfill means wiki entries will miss the latest profile enrichments.
...
**Follow the shared merge protocol at `~/.claude/skills/unbox/references/merge-utils.md` for idempotency checks.** Use the `wiki注入` source_type for markers added to wiki entities.
